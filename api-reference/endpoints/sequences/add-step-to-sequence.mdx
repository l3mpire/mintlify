...
| `sendToAnotherCampaign` | `campaignId` | `leadAction`, `index`, `delay` |
...
<Note>
A transfer that fails — the target campaign is archived, full, or the lead is unsubscribed — always
**pauses** the lead, whatever `leadAction` says, so a lead that never moved does not walk on as if it
had.
</Note>
