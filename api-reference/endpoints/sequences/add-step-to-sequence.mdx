...
| `subject` (String, Optional) | For `email` steps only. The email subject. Omit it or send an empty string on a follow-up so the email replies in the thread of the previous one. Maximum 400 characters (applied to the raw template, including any Liquid syntax) |
