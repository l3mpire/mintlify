...
The request body must be a JSON array of enrichment objects. Each object can contain:

### Input Fields

All input fields are optional, but specific combinations are required depending on the enrichment type requested:

| Field | Type | Description |
|-------|------|-------------|
| `input.linkedinUrl` | string | The LinkedIn URL of the person |
| `input.email` | string | The email of the person |
| `input.firstName` | string | The first name of the person |
| `input.lastName` | string | The last name of the person |
| `input.companyName` | string | The name of the company associated with the person |
| `input.companyDomain` | string | The domain of the company (recommended for better find rate) |
