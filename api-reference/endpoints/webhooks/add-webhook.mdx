...
| ------- | ----- | ---------- |
| `Too many webhooks (max 200 per team, disabled webhooks included)` | The workspace is at the cap. | Free a slot with [Delete Webhook](/api-reference/endpoints/webhooks/delete-webhook). Disabled webhooks count too — see below. |
...
<Warning>
  Uniqueness is on `targetUrl` alone, regardless of `type` and `campaignId`. The
  ...
  on your side), or give each subscription its own URL.
</Warning>

### Disabled webhooks still count toward the cap

When a delivery fails in a way that says the destination is gone — your endpoint answers `404` or `410`, or its host does not resolve — lemlist disables the webhook instead of deleting it, and notifies its owner. [Get Many Webhooks](/api-reference/endpoints/webhooks/get-many-webhooks) keeps returning it, carrying `disabled: true`, `disabledReason: "deliveryError"`, `disabledAt` and `lastErrorStatus`.
...
<Note>
  Since March 30, 2026, a contact unsubscribing emits `entityUnsubscribed` (whole
...
The payload of both is the same shape [Get Contact Sourcing Run](/api-reference/endpoints/contact-sourcing/get-contact-sourcing-run) answers with, under a `data` key.

<Note>
  Subscribe to `contactSourcingFailed` as well as `contactSourcingDone`: a run
  that fails never emits the done event, so an integration listening only for
  the latter waits for a result that is not coming.
</Note>
...
| `leadId` | string | Lead targeted by the activity (campaign events only) |
| `contactId` | string | Contact the activity relates to |
| `leadEmail`, `leadFirstName`, `leadLastName`, `leadCompanyName`, `leadPhone`, `leadPicture` | string | Lead identity as it was when the event happened. Campaign events only, and only the variables the lead had at that moment |
| `email`, `firstName`, `lastName`, `linkedinUrl`, `phone`, `jobTitle`, `companyName`, … | string | Current contact record (its fields, custom fields and company), added at delivery time to every event that has a `contactId`. Never overrides a key already set on the event |
...
<Note>
  **Non-campaign events carry no `lead*` keys.** An activity created from a
  contact page or the inbox rather than from a campaign step (a LinkedIn message
  sent outside a campaign, its opens and replies) has a
  `contactId` but no `leadId`, so `leadFirstName`, `leadLastName` and `leadEmail`
  are absent: there is no lead to take them from. `firstName`, `lastName` and
  `email` are still present, read from the contact. To get a name on every
  event, read `firstName` / `lastName`, or `leadFirstName` / `leadLastName` with
  a fallback to them.
</Note>
