...
By default an existing company receives every non-empty value you send. The `updateStrategy` query parameter changes what happens to the fields it already holds:

| Value | Existing value | Empty value sent |
|-------|----------------|------------------|
| `overwrite` (default) | Replaced | Ignored |
