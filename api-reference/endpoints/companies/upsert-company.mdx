...
Under `fillEmptyOnly`, identifiers (`domain`, `linkedinUrl`, `linkedinUrlSalesNav`, `name`), the owner and the status are kept as well: only the fields the company has empty are filled. A company that already has an owner keeps it, whatever `companyOwner` says; a company that has no owner yet takes the `companyOwner` sent, as it would any other empty field. A newly created company always receives every value sent. Any other value is refused with `400 INVALID_UPDATE_STRATEGY`.
...
On an existing company, what happens to the current owner depends on `updateStrategy`:
...
Sending no `companyOwner` never touches the owner.
