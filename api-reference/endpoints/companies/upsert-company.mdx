...
Under `fillEmptyOnly`, identifiers (`name`, `domain`, `linkedinUrl`, `linkedinUrlSalesNav`), the owner and the status are kept as well when the company already holds them; a missing one is set, like any other empty field. A company that already has an owner keeps it, whatever `companyOwner` says; a company that has no owner yet takes the `companyOwner` sent, as it would any other empty field. A newly created company always receives every value sent. Any other value is refused with `400 INVALID_UPDATE_STRATEGY`.
