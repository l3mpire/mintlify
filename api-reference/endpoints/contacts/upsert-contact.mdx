...
`null` and an absent key never touch a field. Identifiers (`email`, `linkedinUrl`, `linkedinUrlSalesNav`) are never cleared: an empty one is ignored whatever the strategy. To erase a field, send `""` with `updateStrategy=overwrite`.

Under `fillEmptyOnly`, identifiers (`email`, `linkedinUrl`, `linkedinUrlSalesNav`), the company link, the owner and the status are kept as well when the contact already holds them; a missing one is set, like any other empty field. A contact that already has an owner keeps it, whatever `contactOwner` says; a contact that has no owner yet takes the `contactOwner` sent, as it would any other empty field. A newly created contact always receives every value sent. Any other value is refused with `400 INVALID_UPDATE_STRATEGY`.
